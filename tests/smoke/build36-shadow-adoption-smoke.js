(() => {
  ...
  const mode = params.get('b36ShadowSmoke');
  if (!mode) return;
  ...
  async function run() {
    ...
    const reopenMode = mode === 'reopen';
    const adoptionLifecycleOk = firstMode
      ? adoptionStatus.adoptedThisBoot === true && adoptionStatus.existingAtBoot === false
      : reopenMode
        ? adoptionStatus.adoptedThisBoot === false && adoptionStatus.existingAtBoot === true
        : true;

    const preMigration = recovery.preMigration();
    const preMigrationSafe = firstMode
      ? Boolean(preMigration) && (preMigration.values?.[evidenceKey] ?? null) === null && preMigration.kind === 'pre-migration'
      : true;
    ...
    recovery.saveLastGood(`b36-shadow-smoke-${mode}`);
    await wait(30);
