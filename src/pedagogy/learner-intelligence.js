(() => {
  ...
  function errorSnapshot() {
    ...
    return {
      priorities,
      map:new Map(priorities.map(row => [row.item?.id, row]).filter(([id]) => Boolean(id)))
    };
  }
  ...
  function statusWeight(entry) {
    ...
    return { fragile:0.18, new:0.24, learning:0.62, solid:1 }[status] ?? 0.24;
  }
  ...
  function sourceKinds(memoryEntry, errorRow) {
    ...
    const events = errorRow?.entry?.events || [];
    events.forEach(event => kinds.add(sourceKind(event.source)));
  ...
  function itemEvidence(id, snapshots = {}) {
    ...
    const lessonCompleted = completed.has(item.lessonId);
    ...
    const retention = isKnown ? statusWeight(mem) : 0;
    ...
    return {
      id,
      item,
      lesson:LESSON_BY_ID[item.lessonId] || null,
      known:isKnown,
      lessonCompleted,
      reviewed,
      due,
      ...
      retention,
      risk,
      memoryStatus:memoryStatus(mem),
      memory:mem,
      error:err,
      sourceKinds:kinds,
      evidenceChannels:kinds.length
    };
  }
  ...
  function levelEstimate(bands) {
    const b = bands || profile().bands;
    const core = b[4];
    const exchange = b[3];
    const foundations = b[2];
    const daily = b[1];
    const survival = b[0];
  ...
  function recommendationFrom(context) {
    ...
    if (topEvidence && Number(topError.score || 0) >= 8 && (topEvidence.fragile || topEvidence.due)) {
      return {
        type:'review',
        itemId:topEvidence.id,
        lessonId:topEvidence.item.lessonId,
    ...
    if (fragile.length >= 2 || due.length >= 4) {
      ...
      return {
        ...
        lessonId:first?.item?.lessonId || null,
    ...
    return {
      type:'maintain',
      vi:'Không có điểm yếu cấp bách. Giữ nhịp bằng hội thoại, nghe và ôn ngắn.',
      fr:'Aucune fragilité urgente. Entretenir avec conversation, écoute et révisions courtes.'
    };
  }
  ...
  function profile() {
    ...
    const overallScore = Math.round(bands.reduce((sum, band) => sum + band.score * band.items.length, 0) / Math.max(1, bands.reduce((sum, band) => sum + band.items.length, 0)));
    return {
      version:VERSION,
      build:BUILD,
      level,
      ...
      metrics:{
        ...
        knownItems:known.size,
        reviewedItems:reviewed.length,
        due:memory.summary.due?.length || 0,
        fragile:memory.summary.fragile?.length || 0,
        recentDifficulties:window.FrenchTranquilleErrors?.summary?.().recent?.length || 0,
  ...
  function injectProgress() {
    ...
    card.innerHTML = `
      <div class="li-head">
        <div><span class="pill">V2.1 • BUILD 31</span><h2>🧠 ${esc(T('Kế hoạch học thông minh','Plan d’apprentissage intelligent'))}</h2></div>
        <span class="li-level">${esc(p.level.code)}</span>
      </div>
      <p class="li-intro">${esc(T('Mô hình này kết hợp tiến độ, trí nhớ và khó khăn quan sát được. Nó không phải chẩn đoán phát âm hay chứng chỉ CEFR.','Ce modèle combine progression, mémoire et difficultés observables. Ce n’est ni un diagnostic de prononciation ni une certification CECRL.'))}</p>
      <div class="li-summary">
        <div><strong>${p.overallScore}%</strong><span>${esc(T('chỉ số nội bộ','indice interne'))}</span></div>
        <div><strong>${p.confidence}%</strong><span>${esc(T('độ tin cậy','confiance'))}</span></div>
        <div><strong>${p.metrics.reviewedItems}</strong><span>${esc(T('mục có bằng chứng','acquis éprouvés'))}</span></div>
      </div>
      <aside class="li-next li-${esc(p.recommendation.type)}">
        <span>→ ${esc(T('Ưu tiên tiếp theo','Priorité suivante'))}</span>
        <strong>${esc(T(p.recommendation.vi,p.recommendation.fr))}</strong>
      </aside>
      <details class="li-details" ${wasOpen ? 'open' : ''}>
        <summary>${esc(T('Xem cách mô hình suy luận','Voir comment le modèle raisonne'))}</summary>
        <div class="li-band-list">
          ${p.bands.map(band => `<article><div><span>${band.icon}</span><strong>${esc(T(band.vi,band.fr))}</strong><small>${esc(stateLabel(band.state))}</small></div><b>${band.score}%</b><em>${band.confidence}% ${esc(T('tin cậy','confiance'))}</em></article>`).join('')}
        </div>
        <p class="li-note">${esc(T('Một lỗi nhận dạng giọng nói chỉ được coi là tín hiệu của hệ thống nhận dạng. French Trân’quille không biến tín hiệu đó thành điểm phát âm.','Une non-reconnaissance vocale reste un signal du système de reconnaissance. French Trân’quille ne la transforme pas en note de prononciation.'))}</p>
      </details>
    `;
  }
  ...
  function injectSettings() {
    ...
    if (!row) {
      ...
      row.dataset.learnerIntelligenceDiagnostic = '1';
  ...
  window.FrenchTranquilleLearnerIntelligence = API;
  decorate();
})();
