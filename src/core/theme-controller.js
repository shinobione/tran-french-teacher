...
function apply(id){const t=valid(id);document.documentElement.dataset.theme=t;if(localStorage.getItem(KEY)!==t)localStorage.setItem(KEY,t);sync(t);window.FrenchTranquillePremiumV55?.refresh?.();window.FrenchTranquillePremiumV59Interactions?.refresh?.();window.FrenchTranquillePremiumV59LessonLayout?.refresh?.();window.FrenchTranquillePremiumV59System?.refresh?.();return t}
...
function setPickerOpen(p,open){if(!(p instanceof HTMLDetailsElement))return false;const next=!!open;p.open=next;p.dataset.themePickerOpen=next?'1':'0';const summary=p.querySelector(':scope>summary');if(summary){summary.setAttribute('role','button');summary.setAttribute('aria-expanded',next?'true':'false');summary.setAttribute('tabindex','0')}return next}
function picker(){const host=document.querySelector('.screen-settings .narrow'),tpl=document.getElementById('ft-theme-settings');if(!host||!tpl)return;let p=host.querySelector(':scope>.ft-theme-settings-inline');if(!p){p=tpl.cloneNode(true);p.removeAttribute('id');p.removeAttribute('aria-hidden');p.classList.add('ft-theme-settings-inline');p.style.removeProperty('display');const v=p.querySelector('summary>span:last-child');if(v)v.dataset.themeCurrent='1';host.prepend(p)}tpl.style.display='none';tpl.setAttribute('aria-hidden','true');setPickerOpen(p,p.open);sync(current())}
function mount(){brand();picker();sync(current());window.FrenchTranquillePremiumV55?.refresh?.();window.FrenchTranquillePremiumV59Interactions?.refresh?.();window.FrenchTranquillePremiumV59LessonLayout?.refresh?.();window.FrenchTranquillePremiumV59System?.refresh?.()}
function pickerSummary(target){return target?.closest?.('.screen-settings .ft-theme-settings-inline>summary')||null}
