(() => {
  ...
  async function play({slow=false}={}){
    if(!currentExercise||speaking)return;
    ...
    if(currentExercise.family==='dialogue'){
      for(let i=0;i<currentExercise.lines.length;i+=1){
        const line=currentExercise.lines[i];
        await utter(line.fr,{slow,pitch:line.speaker==='A'?.97:1.06});
      }
    }else await utter(currentExercise.spoken,{slow});
  ...
  function closeListening(){
    speechSynthesis?.cancel?.();speaking=false;overlay?.remove();overlay=null;document.documentElement.classList.remove('listening-open');scheduleDecorate();
  }
  ...
  document.addEventListener('keydown',event=>{if(event.key==='Escape'&&overlay)closeListening()});
  ...
  window.FrenchTranquilleListening={version:'1.13.0',build:20,key:KEY,state:()=>state,availability,open:openListening,close:closeListening};
})();
