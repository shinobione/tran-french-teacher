...
def fix_workflows():
    ...
    for p in sorted(WF_DIR.glob('*.yml')):
        if p.name.startswith('_repo-layout-ci-semantics'): continue
        text=p.read_text(encoding='utf-8'); lines=[]
        for line in text.splitlines(keepends=True):
            original=line
            if 'src/premium/premium-theme-polish.css' in line:
                line=re.sub(
                    r"(['\"])src/premium/(premium-[^'\"]+\.css(?:\?[^'\"]*)?)\1(?=[^\n]*src/premium/premium-theme-polish\.css)",
                    lambda m:f"{m.group(1)}./{m.group(2)}{m.group(1)}", line)
            ...
            if line!=original: total+=1
            lines.append(line)
        out=''.join(lines)
        if out!=text:
            p.write_text(out,encoding='utf-8'); touched.append(p)
...
def fix_tool_requires():
    touched=[]; pattern=re.compile(r"require\((['\"])(\.\./([^/'\"]+\.(?:js|json)))\1\)")
    for p in sorted(Path('tools').rglob('*')):
        if not p.is_file() or p.suffix not in {'.js','.cjs','.mjs'}: continue
        text=p.read_text(encoding='utf-8')
        def repl(m):
            quote,raw,basename=m.group(1),m.group(2),m.group(3)
            if (p.parent/raw).resolve().exists(): return m.group(0)
        ...
        if out!=text: p.write_text(out,encoding='utf-8'); touched.append(p)
...
def fix_browser_harnesses():
    ...
    for p in sorted(BROWSER_DIR.glob('*.html')):
        text=p.read_text(encoding='utf-8'); out=text
        ...
        if p.name=='premium-v510-practice-icons-tribunal.html':
            # Values in `assets` are compared against getAttribute('src') from the APP iframe.
            # Keep them app-document-relative even though this tribunal itself moved under tests/browser.
...
def verify():
    ...
    for p in WF_DIR.glob('*.yml'):
        if p.name.startswith('_repo-layout-ci-semantics'): continue
        text=p.read_text(encoding='utf-8')
        if '/tmp/tests/browser/' in text or '/tmp/tests/smoke/' in text or '/tmp/src/' in text: bad.append(str(p))
    if bad: fail('stale migrated /tmp paths: '+', '.join(bad))

    for p in WF_DIR.glob('*.yml'):
        if p.name.startswith('_repo-layout-ci-semantics'): continue
        for line in p.read_text(encoding='utf-8').splitlines():
            if 'tail -n 1 ./premium-theme-polish.css' in line: fail(f'tail operand incorrectly made relative in {p}: {line}')
            if "@import url('./src/premium/" in line: fail(f'over-prefixed @import expectation remains in {p}: {line}')

    meta=Path('src/core/build-meta.js').read_text(encoding='utf-8'); bridge=Path('src/core/runtime-bridge.js').read_text(encoding='utf-8')
    for needle,label in [
        ("import './runtime-contracts.js?v=2.0.0-b30'",'build-meta contracts'),
        ("import './runtime-bridge.js?v=2.0.0-b30'",'build-meta bridge')]:
        if needle not in meta: fail(label+' import changed')
    if "from './runtime-contracts.js?v=2.0.0-b30'" not in bridge: fail('runtime-bridge relative contract import changed')
    for wf in ('build30-architecture-hardening.yml','v2-release-freeze.yml'):
        text=(WF_DIR/wf).read_text(encoding='utf-8')
        if 'src/core/runtime-contracts.js?v=2.0.0-b30' in text or 'src/core/runtime-bridge.js?v=2.0.0-b30' in text: fail(f'Build30 assertion still over-prefixed in {wf}')

    broken=[]; attr=re.compile(r'(?:src|href)=["\']([^"\']+)["\']')
    for p in BROWSER_DIR.glob('*.html'):
        for raw in attr.findall(p.read_text(encoding='utf-8')):
            if raw.startswith(('http:','https:','data:','#','javascript:')): continue
            path=raw.split('?',1)[0].split('#',1)[0]
            if path and not (p.parent/path).resolve().exists(): broken.append(f'{p}:{raw}')
    if broken: fail('broken browser src/href refs: '+', '.join(broken[:50]))

    nav=Path('tests/smoke/field-navigation-v3-smoke.js').read_text(encoding='utf-8')
    if "frame.src='../../index.html?uxSmoke=lesson8&fieldNavV3=1'" not in nav: fail('Field Navigation smoke app URL wrong')
    icons=Path('tests/browser/premium-v510-practice-icons-tribunal.html').read_text(encoding='utf-8')
    for key,file in [('review','review-premium.webp'),('listening','listen-premium.webp')]:
        if not re.search(rf"\b{key}\s*:\s*['\"]\./assets/premium/practice/{re.escape(file)}['\"]",icons):
            fail(f'V5.10 tribunal {key} literal src expectation wrong')
    if '../../assets/premium/practice/' in icons: fail('V5.10 tribunal still has tribunal-relative approved artwork expectations')

    for base in (Path('src'),SMOKE_DIR):
        for p in base.rglob('*.js'):
            r=subprocess.run(['node','--check',str(p)],capture_output=True,text=True)
            if r.returncode: fail(f'node --check failed {p}: {r.stderr.strip()}')
    r=subprocess.run(['node','tools/test-build28-data-recovery.cjs'],capture_output=True,text=True)
    if r.returncode: fail('Build28 pure recovery test failed: '+r.stderr.strip())
    ...
    if r.returncode: fail('protected sanctuary diff detected')


def stage_workflows():
    if STAGE_DIR.exists(): subprocess.run(['rm','-rf',str(STAGE_DIR)],check=True)
    STAGE_DIR.mkdir(parents=True)
    for p in WF_DIR.glob('*.yml'):
        if not p.name.startswith('_repo-layout-ci-semantics'):
...
def main():
    fix_workflows(); fix_tool_requires(); fix_browser_harnesses(); fix_smoke_runtime_urls(); verify(); stage_workflows(); note('consolidated semantics migration verified')
