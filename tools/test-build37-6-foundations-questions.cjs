...
assert.ok(pilot.includes("systematization:'37.6'"), '37.6 systematization marker missing');
...
for (const forbidden of ['F08:engine.compile','F13:engine.compile','F16:engine.compile']) {
  assert.equal(capsuleSource.includes(forbidden), false, `37.6 must not mass-rollout ${forbidden.split(':')[0]}`);
}
...
console.log('Build 37.6 F12 practical-question capsule: PASS');
