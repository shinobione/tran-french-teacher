...
STATE = '''# French Trân’quille — CURRENT PROJECT STATE
...
CHECKPOINT = '''# 0. Canonical checkpoint — 2026-08-16
...
Path('PROJECT-STATE.md').write_text(STATE, encoding='utf-8')
roadmap_path = Path('MASTER-ROADMAP.md')
roadmap = roadmap_path.read_text(encoding='utf-8')
updated, count = re.subn(
    r'# 0\. Canonical checkpoint — 2026-08-16.*?(?=\n# 1\.)',
    CHECKPOINT.rstrip(),
    roadmap,
    flags=re.S,
)
if count != 1:
    raise SystemExit(f'expected one canonical checkpoint, got {count}')
