...
const LESSONS = [
  ...
  },
  {
    ...
    items: [
      {id:'svp', fr:"S'il vous plaît.", vi:'Làm ơn / vui lòng.'},
      {id:'excusez-moi', fr:'Excusez-moi.', vi:'Xin lỗi / làm phiền một chút.'},
      {id:'je-comprends', fr:'Je comprends.', vi:'Tôi hiểu.'},
      {id:'je-ne-comprends-pas', fr:'Je ne comprends pas.', vi:'Tôi không hiểu.'},
      {id:'repetez', fr:'Pouvez-vous répéter ?', vi:'Bạn có thể nhắc lại không?'},
      {id:'plus-lentement', fr:"Plus lentement, s'il vous plaît.", vi:'Chậm hơn một chút, làm ơn.'}
    ],
  ...
  },
  {
    ...
    introVi: 'Bây giờ bạn có thể nói thêm vài điều thật về bản thân.',
    introFr: 'Maintenant, tu peux dire quelques choses simples et vraies sur toi.',
    items: [
      {id:'je-suis-vietnamienne', fr:'Je suis vietnamienne.', vi:'Tôi là người Việt Nam.'},
      {id:'je-viens-vietnam', fr:'Je viens du Vietnam.', vi:'Tôi đến từ Việt Nam.'},
      {id:'jhabite-hcm', fr:"J'habite à Hô Chi Minh-Ville.", vi:'Tôi sống ở Thành phố Hồ Chí Minh.'},
      {id:'et-vous', fr:'Et vous ?', vi:'Còn bạn thì sao?'},
      {id:'oui', fr:'Oui.', vi:'Vâng / có.'},
      {id:'non', fr:'Non.', vi:'Không.'}
    ],
  ...
  },
  {
    ...
    introVi: 'Một tình huống rất thực tế: gọi đồ uống mà không cần dịch từng từ.',
    introFr: 'Situation très concrète : commander une boisson sans traduire chaque mot.',
    items: [
      {id:'je-voudrais', fr:'Je voudrais…', vi:'Tôi muốn… (lịch sự).'},
      {id:'eau', fr:"De l'eau.", vi:'Nước.'},
      {id:'cafe', fr:'Un café.', vi:'Một ly cà phê.'},
      {id:'the', fr:'Un thé.', vi:'Một ly trà.'},
      {id:'addition', fr:"L'addition, s'il vous plaît.", vi:'Cho tôi xin hóa đơn.'},
      {id:'combien-ca-coute', fr:'Combien ça coûte ?', vi:'Cái này giá bao nhiêu?'}
    ],
  ...
  },
  {
    ...
    introVi: 'Những câu rất ngắn nhưng giúp bạn bắt đầu nói về sở thích của mình.',
    introFr: 'Des phrases très courtes pour commencer à parler de tes goûts.',
    items: [
      {id:'jaime', fr:"J'aime…", vi:'Tôi thích…'},
      {id:'je-naime-pas', fr:"Je n'aime pas…", vi:'Tôi không thích…'},
      {id:'beaucoup', fr:'Beaucoup.', vi:'Rất nhiều.'},
      {id:'un-peu', fr:'Un peu.', vi:'Một chút.'},
      {id:'cest-bon', fr:"C'est bon.", vi:'Ngon / tốt.'},
      {id:'je-prefere', fr:'Je préfère…', vi:'Tôi thích … hơn.'}
    ],
...
function initialState(){
  return {
    schemaVersion:2,
    learnerName:'Trân', level:'A0',
    lessonProgress:Object.fromEntries(LESSONS.map(l => [l.id,0])),
    completedLessons:[], knownItems:[], reviewState:{},
    conversationWins:0, lastActivity:null,
...
function migrateState(parsed){
  ...
  if (parsed.schemaVersion === 1 || 'lesson1Step' in parsed) {
    const completed = Boolean(parsed.lesson1Completed);
    return {
      ...base,
      knownItems:Array.isArray(parsed.knownItems)?parsed.knownItems:[],
      reviewState:parsed.reviewState||{},
      conversationWins:Number(parsed.conversationWins||0),
      lastActivity:parsed.lastActivity||null,
      streak:{...base.streak,...(parsed.streak||{})},
      lessonProgress:{...base.lessonProgress,l1:completed?999:Number(parsed.lesson1Step||0)},
      completedLessons:completed?['l1']:[]
    };
  }
  return base;
}
...
function top(title,{back=false,settings=false}={}){return `<header class="topbar"><button class="icon-btn ${back?'':'invisible'}" data-back aria-label="${h(T('Quay lại','Retour'))}">‹</button><div><div class="eyebrow">FRENCH TRÂN’QUILLE</div><h1>${h(title)}</h1></div><button class="icon-btn ${settings?'':'invisible'}" data-go="settings" aria-label="${h(T('Cài đặt','Réglages'))}">⚙</button></header>`}
