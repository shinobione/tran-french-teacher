...
Build40 rejected opening A2 because the product already had broad A1 thematic coverage while productive generalization and trustworthy construction evidence were narrower.

It selected **A1 Productive Consolidation** with four important roadmap principles:

1. audit deterministic, vocabulary-safe productive families first;
2. give F16 `à / de` contractions a dedicated audit because it is the clearest remaining `teach-core` Foundation debt;
...
Build41.3 places those three deterministic reconstructions after normal lesson-36 teaching and preserves the existing Build38 shared Transfer renderer, historical routes and conservative evidence model.
...
| Past-time recombination | IMPLEMENTABLE — narrow recent-past family selected |
...
Build39 deliberately supports reliable advice only for:
...
The following remain unavailable as trustworthy durable learner-intelligence families:
...
Build41 added meaningful productive events but did not add a durable evidence owner, and that was correct. A three-item ephemeral Transfer family is not enough evidence to claim durable grammatical mastery.
...
→ only if justified: coherent learner-facing teach-core owner
→ only after that: reassess whether a deterministic productive family exists
```
...
- 41.1 audited the full candidate set selected by Build40;
- exactly one candidate passed the strict deterministic contract;
- 41.2 certified its pure core;
- placement was separately audited;
- 41.3 delivered and CI-certified the learner-facing integration;
- no unimplemented Build41 family remains authorized by evidence.

Canonical milestone result:

```text
Build41 — A1 Productive Consolidation family #1
CLOSED / SUCCESSFUL NARROW CONSOLIDATION
```

This closure does **not** mean A1 productive depth is complete.
...
- productive depth widened only narrowly;
- Transfer/concept durable evidence did not change;
- explicit F16 teaching debt remains;
- no fresh A1 readiness audit has yet justified moving to A2.
...
- inspect the real 52/313 curriculum for already-taught `à / de` contraction anchors;
- classify which forms are explicit, distributed, lexical or missing;
- identify the earliest valid teaching placement if a coherent Foundation owner is justified;
- decide whether existing learner-facing Foundation infrastructure can be reused;
- decide whether any later deterministic productive family becomes admissible.

Hard boundaries:

- docs/read-only audit only;
- no curriculum mutation;
- no Foundation capsule implementation yet;
- no Transfer implementation yet;
- no store/schema/Recovery/Evidence change;
- no mastery claim;
