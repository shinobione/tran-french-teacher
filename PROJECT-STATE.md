...
- Last verified `main` before publication of this closeout: **`448d5e5876aecda67e4b75f987f058751cc7ca2e`**.
- That commit is the squash merge of PR **#249 — `A2 · design first bridge lane as R1 reception`**, GitHub **verified=true / reason=valid**.
- PR #249 accepted exact head: **`52ba8e8b94e94493f143d8430aee83dff829efb1`**.
- Published closeout control candidate: PR **#250 — `Docs · close accepted A2 bridge design audit`**, branch **`docs/a2-bridge-design-closeout`**.
- **GitHub live metadata owns #250's moving head/open-or-merged state and the current `main` tip.** This file intentionally records the last verified pre-publication `main` instead of predicting a future merge SHA.
- Canonical accepted A2 scope audit: `docs/A2-ENTRY-SCOPE-AUDIT.md`.
- Canonical accepted bridge design: `docs/A2-BRIDGE-DESIGN-AUDIT.md` plus normative review amendment `docs/A2-BRIDGE-DESIGN-AUDIT-P2-FACT-IDENTITY-AMENDMENT.md`.
...
No new lesson number or item is authorized by that audit.
...
pilot fact/item IDs          jai-mal-ventre / depuis-hier / rendez-vous-medecin
...
Deferred lanes remain valid but unselected:

```text
A2-P1  DEFER — needs honest short-series completion/evidence policy
A2-I1  DEFER — needs carried-fact state contract and transition semantics
A2-W1  DEFER — needs dedicated writing owner and non-fake free-text evaluation policy
```

### Accepted Codex P2 fact-identity boundary

The original design's unique question IDs were insufficient to prove a **multi-fact** task. The accepted normative amendment therefore locks:

```text
2–4 deterministic questions per bridge activity
question.id stable and unique
question.factId mandatory and stable
factId unique across the bridge activity
factId must reference an accepted existing prerequisite/source item
...
one canonical deterministic answer index per question
explicit source facts only; no semantic free-text grading
```

For the canonical `doctor-appointment` pilot, the three distinct fact IDs are exactly:
...
## NEXT — A2-R1 Pure Multi-Fact Reception Contract Proof

Only after PR #250 closeout acceptance, the next authorized slice is one **pure deterministic contract proof**, still **NOT STARTED** until the next delegated/explicit command.

Expected narrow scope:

```text
new pure core: src/pedagogy/a2-reception-bridge-core.js
...
```text
valid 3-question doctor-appointment fixture with 3 distinct factIds
reject fewer than 2 or more than 4 questions
reject duplicate question IDs
reject duplicate factIds / duplicate fact targets
reject duplicate normalized prompts
reject invalid answer index/options
reject missing/invalid prerequisite/evidence references
require evidenceItems == [factId] for the first proof
return detached/deeply frozen output
preserve existing single-question source fields
use no DOM/storage/network APIs or side effects
```

It must **not** touch or authorize:

```text
learner-facing UI
listening-engine integration
listening-data mutation
curriculum mutation
new lessons / renumbering
route/navigation changes
localStorage / IndexedDB / network
Memory / Error Intelligence / Evidence writes
P3b durability
new durable store
Build43
learner A2 unlock/readiness/mastery
CEFR certification
```
...
```text
Read live GitHub state for PR #250 and main first.

IF #250 is still OPEN:
...
→ then A2-R1 Pure Multi-Fact Reception Contract Proof is NEXT / NOT STARTED

IF #250 is already MERGED:
→ verify the actual main SHA/signature from GitHub live state
→ treat A2 Bridge Design Audit and its closeout as accepted
