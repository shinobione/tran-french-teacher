(()=>{
  ...
  let step='boot';
  async function press(w,button,delay=280){if(!button)throw Error(`${step}:missing-button`);button.dispatchEvent(new w.MouseEvent('click',{bubbles:true,cancelable:true}));await wait(delay)}
  ...
  frame.addEventListener('load',()=>setTimeout(async()=>{
    try{
      ...
      step='home-quick-listening-close';await press(w,d.querySelector('[data-listening-close]'),260);assertDestination(w,d,'home');
      ...
      for(let n=0;n<5;n++){
        ...
        await press(w,listen,320);if(!visible(w,d.querySelector('.listening-overlay')))throw Error(`${step}:not-visible`);
